...
Este capítulo detalha a solução implementada: um aplicativo para sugestão de trocas alimentares para pessoas com Transtorno do Espectro Autista (TEA). A aplicação foi desenvolvida para apoiar cuidadores e também pessoas com TEA independentes no manejo da seletividade alimentar.

O capítulo descreve os resultados da solução final. A seção "Sobre o Aplicativo" define seus objetivos funcionais . A "Identidade Visual" apresenta os elementos de design. A "Interface do Aplicativo"  detalha as telas implementadas. Por fim, "Organização dos Dados"  descreve a arquitetura do banco de dados e a "Regra de Negócio" define o algoritmo de sugestão.
...
\section{Estrutura do Código do Projeto}
